# -*- coding: utf-8 -*-
"""Gradient_descent_final.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1aMHHstwY9iwS0M5pbKXGGaOjFx4IBznw
...
def Rastrigin_grad(X): 
  grad = []
  for x in X:
    grad.append(2*x+20*math.pi*jnp.sin(2*math.pi*x))
...
def gradient(cur_x, gradient_funkcji, precision, max_iters, rate):
  ...
  while max(previous_step_size) > precision and iters < max_iters:
      ...
      cur_x = prev_x - rate * gradient_funkcji(prev_x) #Grad descent
...
for xi in range(11):
  ...
  for i in range(5):
    cur_x = np.random.uniform(0,1,xn) #inicjalizacja początkowych wartości zmiennych decyzyjnych
    # czas_1 = 0
    # czas_2 = 0
    # czas_3 = 0
    ...
    czas_1_total[xi][i],x_1=gradient(cur_x, Rastrigin_grad, precision, max_iters, rate)
    czas_2_total[xi][i],x_2=gradient(cur_x, Sphere_grad, precision, max_iters, rate)
    czas_3_total[xi][i],x_3=gradient(cur_x, Rosenbrock_grad, precision, max_iters, 0.0007)
    if i<1:
      all_x1 = x_1
      all_x2 = x_2
      all_x3 = x_3
...
def gradient_draw(gradient_funkcji, funkcja, resolution, precision, max_iters):
  cur_x = np.random.uniform(-2,2,2)
  rate = 0.001 # Learning rate
  max_iters = 10000 
  iters = 0 #iteration counter
  previous_step_size =[0.5]
  ...
  while max(previous_step_size) > precision and iters < max_iters:
      ...
      prev_x = cur_x #Store current x value in prev_x
      cur_x = cur_x - rate * gradient_funkcji(prev_x) #Grad descent
      previous_step_size = abs(cur_x - prev_x) #Change in x
      iters = iters+1 #iteration count
      #print("Iteration",iters,"\nX value is",cur_x) #Print iterations
...
gradient_draw(Rastrigin_grad,rastrigin,10 , 0.0000001, 2000)
gradient_draw(Sphere_grad,sphere,50 , 0.00001, 2000)
gradient_draw(Rosenbrock_grad,rosenbrock,100 , 0.00001, 2000)